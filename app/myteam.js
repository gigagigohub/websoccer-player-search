...
function pointSourceFontSizeRem(label) {
  ...
  const targetRem = mobile ? 11.1 : 12.4;
  return Math.max(0.31, Math.min(0.5, targetRem / Math.max(units, 1)));
}
...
function renderLineup() {
  ...
  const renderSlotRange = (start, end) => lineup.slice(start, end).map((entry, localIdx) => {
    ...
    const pointSourceLabel = pointEstimated && !lifecycleModeEnabled ? String(v4PointInfo?.label || "estimated") : "";
