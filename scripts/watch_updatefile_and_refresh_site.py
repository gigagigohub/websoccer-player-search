...
DEFAULT_PUSHOVER_CONFIG = Path.home() / ".yamato_pushover_watch" / "config.json"
...
def load_pushover_config(path: Path) -> dict[str, str]:
    ...
    if not token or not user:
        raise ValueError(f"Pushover token/user key missing in {path}")
    return {"token": token, "user": user}


def notify(config_path: Path, title: str, message: str, enabled: bool, log_path: Path) -> None:
    if not enabled:
        return
    try:
        cfg = load_pushover_config(config_path)
