...
LABEL = "com.gigagigo.websoccer.updatefile-core-watch"
REPO_ROOT = Path(__file__).resolve().parents[1]
WRAPPER = REPO_ROOT / "scripts" / "run_updatefile_and_core_data_watch.sh"
RUNTIME_DIR = Path.home() / "Library" / "Application Support" / "websoccer-player-search"
RUNTIME_WRAPPER = RUNTIME_DIR / "run_updatefile_and_core_data_watch.sh"
PLIST_PATH = Path.home() / "Library" / "LaunchAgents" / f"{LABEL}.plist"
LOG_DIR = Path.home() / "Library" / "Logs" / "websoccer-player-search"

RUN_HOURS = [0, 1, 2, 3, *range(7, 24)]
...
def main() -> int:
    ...
    plist = {
        ...
        "StartCalendarInterval": [{"Hour": hour, "Minute": 0} for hour in RUN_HOURS],
        "WorkingDirectory": str(REPO_ROOT),
        "StandardOutPath": str(LOG_DIR / "updatefile-core-watch.out.log"),
        "StandardErrorPath": str(LOG_DIR / "updatefile-core-watch.err.log"),
        "RunAtLoad": False,
    ...
    print("[DONE] schedule: hourly at minute 0, excluding 04:00, 05:00, and 06:00 JST")
    print(f"[DONE] logs: {LOG_DIR / 'updatefile-core-watch.out.log'}")
    return 0
